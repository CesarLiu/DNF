...
class PETRDNFHead(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is PETRDNFHead):
        ...
        super(PETRDNFHead, self).__init__(num_classes, in_channels, init_cfg = init_cfg)
    ...
    # modify prepare_for_dn for denoising as fusion of object list
    def prepare_for_dn(self, batch_size, reference_points, img_metas=None, objectlist_bbox=None):
        ...
        if self.training:
            ...
            if self.bbox_noise_scale > 0:
                # box size noise
                # Generate noise for the bounding box scale (width, height, depth)
                rand_prob_scale = torch.rand_like(known_bbox_scale) * 2 - 1.0  # Random noise in range [-1, 1]
                known_bbox_scale += torch.mul(rand_prob_scale, known_bbox_scale) * self.bbox_noise_scale
                # box center noise
                ...
                noisy_indices = torch.nonzero(mask, as_tuple=True)[0]  # Indices of noisy labels
                noisy_labels = known_labels[noisy_indices]  # Extract noisy labels
                # Identify labels belonging to each group
                vehicle_mask = noisy_labels <= 4  # Vehicle group (indices 0-4)
                others_mask = noisy_labels > 4    # Others group (indices 5-9)
                # Define possible replacements
                vehicle_group_with_unknown = torch.tensor([0, 1, 2, 3, 4, 10], device=known_labels.device)
                others_group_with_unknown = torch.tensor([5, 6, 7, 8, 9, 10], device=known_labels.device)
                # Randomly select replacements
                vehicle_replacements = vehicle_group_with_unknown[
                    torch.randint(len(vehicle_group_with_unknown), (vehicle_mask.sum(),))
                ]
                others_replacements = others_group_with_unknown[
                    torch.randint(len(others_group_with_unknown), (others_mask.sum(),))
                ]
                # Apply replacements
                noisy_labels[vehicle_mask] = vehicle_replacements
                noisy_labels[others_mask] = others_replacements

                # Assign updated labels back to known_labels
                known_labels[noisy_indices] = noisy_labels
        ...
        if self.with_dnf:
            ...
            # for denoising, need to calculate the attn_mask
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is PETRDNFHead:
            convert_dict = {
