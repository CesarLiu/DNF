...
class PETRv2DNHead_version1_test(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is PETRv2DNHead_version1_test):
        ...
        self.with_gauss_pe = with_gauss_pe
        self.with_time = with_time
        self.with_multi = with_multi
        self.group_reg_dims = group_reg_dims
        super(PETRv2DNHead_version1_test, self).__init__(num_classes, in_channels, init_cfg = init_cfg)
    ...
    def _init_layers(self):
        ...
        if self.with_gauss_pe:
            self.gauss_pe = ConvLayer(1,self.embed_dims,16,16,0)
    ...
    def prepare_for_dn(self, batch_size, reference_points, img_metas, objectlist_bbox):
        #if self.training:            
        ...
        if self.bbox_noise_scale > 0 and self.training:
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is PETRv2DNHead_version1_test:
            convert_dict = {
    ...
    def forward(self, mlvl_feats, img_metas, objectlist_bbox, objectlist_bbox_corner=None):
        """Forward function.
        Args:
        ...
        if self.with_position:
            ...
            if self.with_fpe:
                fpe_coords_position_embeding = self.fpe(coords_position_embeding.flatten(0,1), x.flatten(0,1)).view(x.size())
                pos_embed = fpe_coords_position_embeding
            if self.with_gauss_pe:
                ...
                gauss_coords_position_embeding = self.gauss_pe(coords_position_embeding.flatten(0,1), gaussian_masks.flatten(0,1)).view(x.size())
                pos_embed = gauss_coords_position_embeding
            if self.with_fpe and self.with_gauss_pe:
                pos_embed = fpe_coords_position_embeding + gauss_coords_position_embeding
