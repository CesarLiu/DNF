...
class ConvLayer(nn.Module):
    ...
    def forward(self, x, g_mask):
        # # x should have shape [batch_size, num_cams, height, width]
        # batch_size, num_cams, height, width = g_mask.shape
        
        # # Add a dimension to match Conv2d input requirements
        # g_mask = g_mask.unsqueeze(2)  # shape becomes [batch_size, num_cams, 1, height, width]
        
        # # Reshape to [batch_size * num_cams, in_channels, height, width]
        # g_mask = g_mask.view(batch_size * num_cams, 1, height, width)
        
        # Apply the convolution
        g_mask = self.conv(g_mask)
...
class PETRv2DNHeadGM(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is PETRv2DNHeadGM):
        ...
        super(PETRv2DNHeadGM, self).__init__(num_classes, in_channels, init_cfg = init_cfg)
    ...
    def _init_layers(self):
        ...
        if self.with_fpe:
            # self.fpe = ConvLayer(1,self.embed_dims,16,16,0)
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is PETRv2DNHeadGM:
            convert_dict = {
    ...
    def forward(self, mlvl_feats, img_metas, objectlist_bbox, objectlist_bbox_corner):
        """Forward function.
        Args:
        ...
        if self.with_position:
            ...
            if self.with_fpe:
                ...
                # create gaussian mask for object list
                #gaussian_mask for object list
                ...
                #     for cam_id in range(num_cams):
                #         img_h, img_w, _ = img_metas[img_id]['img_shape'][cam_id]                 
                #         lidar2img_rt = img_metas[img_id]['lidar2img'][cam_id]
                ...
                #         coor_x = imgfov_pts_2d[: , :, 0]#(num_bbox, 8)
                #         coor_y = imgfov_pts_2d[: , :, 1]
                #         center_y = ((np.min(coor_y, axis=1) + np.max(coor_y, axis=1)) / 2).reshape(-1, 1)
                #         center_x = ((np.min(coor_x, axis=1) + np.max(coor_x, axis=1)) / 2).reshape(-1, 1)
                ...
                #         imgfov_pts_2d = torch.tensor(imgfov_pts_2d).to(x.device)
                #         centers = torch.cat([torch.tensor(center_xs).to(x.device), torch.tensor(center_ys).to(x.device)], dim=-1).int()  # center on the feature map
                #         corners = (imgfov_pts_2d[on_the_image].max(1).values - imgfov_pts_2d[on_the_image].min(1).values) 
                ...
                #         distance = (centers[:, None, :] - (img_pos - 0.5)).norm(dim=-1) ** 2
                #         gaussian_mask = (-distance / (2 * sigma[:, None] ** 2)).exp()
                #         gaussian_mask[gaussian_mask < torch.finfo(torch.float32).eps] = 0
                #         for i in range(centers.size(0)):
                #             gaussian_masks[img_id, cam_id, :]  += gaussian_mask[i]
                #         gaussian_masks[img_id, cam_id, :] /= gaussian_masks[img_id, cam_id, :].max()

                # gaussian_masks = gaussian_masks.reshape(batch_size, num_cams,1,input_img_h, input_img_w)
                # coords_position_embeding = self.fpe(coords_position_embeding.flatten(0,1), gaussian_masks.flatten(0,1)).view(x.size())
        ...
        reference_points, attn_mask, mask_dict = self.prepare_for_dn(batch_size, reference_points, img_metas, objectlist_bbox)
        ...
        if self.training and seed > self.mask_mode:
            ...
            gaussian_masks = F.interpolate(
                gaussian_masks, size=x.shape[-2:])
            gaussian_masks = gaussian_masks.reshape(batch_size, -1)  # 变为 [bs, n*h*w]
            gaussian_masks = gaussian_masks.unsqueeze(1).expand(-1, self.num_query, -1)  # 扩展为 [bs, self.num_query, n*h*w]
            gaussian_masks = torch.cat((torch.zeros(batch_size, query_embeds.size(1) - self.num_query, gaussian_masks.size(2)).to(x.device), gaussian_masks), dim=1) # [bs, num_denoisy+num_query, n*h*w]
            #gaussian_masks = gaussian_masks.unsqueeze(1).expand(-1, query_embeds.size(0), -1)

            gaussian_masks = (gaussian_masks+1).log()
            #########################
            # attn_mask = None
            # gaussian_masks = None  
        outs_dec, _ = self.transformer(x, masks, query_embeds, pos_embed, gaussian_masks, attn_mask, self.reg_branches)
        ...
        #separete the prediction of dn task and matching task
        if mask_dict and mask_dict['pad_size'] > 0:
            #get the prediction of dn task
            ...
            #let outputs_class & outputs_coord keep the original prediction of matching task, compatible with the original architecture
            outputs_class = all_cls_scores[:, :, mask_dict['pad_size']:, :]
            outputs_coord = all_bbox_preds[:, :, mask_dict['pad_size']:, :]
            #recoding the prediction of dn task to mask_dict
    ...
    def dn_loss_single_match(self,
        ...
        bbox_preds_list = [bbox_preds[i] for i in range(num_imgs)]
        cls_reg_targets = self.get_targets(cls_scores_list, bbox_preds_list,
                                           known_bboxs, known_labels, 
                                           num_total_pos)
        (labels_list, label_weights_list, bbox_targets_list, bbox_weights_list,
         num_total_pos, num_total_neg) = cls_reg_targets
        
        known_labels = torch.cat(labels_list, 0)
        known_bboxs = torch.cat(bbox_targets_list, 0)
        bbox_weights = torch.cat(bbox_weights_list, 0)
        label_weights = torch.cat(label_weights_list, 0)
        # construct weighted avg_factor to match with the official DETR repo
        cls_avg_factor = num_total_pos * 1.0 + \
    ...
    def loss(self,
        ...
        if preds_dicts['dn_mask_dict'] is not None:
            ...
            dn_losses_cls, dn_losses_bbox = multi_apply(
                ...
                all_num_tgts_list)
            # dn_losses_cls, dn_losses_bbox = multi_apply(
            #     self.dn_loss_single_match, output_known_class.unsqueeze(1), output_known_coord.unsqueeze(1),
            #     all_known_bboxs_list, all_known_labels_list, 
            #     all_num_tgts_list)
