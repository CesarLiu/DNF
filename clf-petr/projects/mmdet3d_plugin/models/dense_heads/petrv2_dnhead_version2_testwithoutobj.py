...
class PETRv2DNHeadGM_testwithout(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is PETRv2DNHeadGM_testwithout):
        ...
        super(PETRv2DNHeadGM_testwithout, self).__init__(num_classes, in_channels, init_cfg = init_cfg)
    ...
    def prepare_for_dn(self, batch_size, reference_points,  objectlist_bbox):
        if self.training:            
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is PETRv2DNHeadGM_testwithout:
            convert_dict = {
    ...
    def forward(self, mlvl_feats, img_metas, objectlist_bbox, objectlist_bbox_corner):
        ...
        reference_points, attn_mask, mask_dict = self.prepare_for_dn(batch_size, reference_points, objectlist_bbox)
        ...
        #gaussian_masks_dn = torch.zeros(batch_size, query_embeds.size(1)- self.num_query, gaussian_masks.size(2)).to(x.device)#[bs, num_denoisy, n*h*w]
        #gaussian_masks = torch.cat((gaussian_masks_dn, gaussian_masks), dim=1) # [bs, num_denoisy+num_query, n*h*w]
