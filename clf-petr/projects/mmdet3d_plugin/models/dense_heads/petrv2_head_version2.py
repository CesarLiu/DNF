...
class PETRv2Head_version2(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is PETRv2Head_version2):
        ...
        super(PETRv2Head_version2, self).__init__(num_classes, in_channels, init_cfg = init_cfg)
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is PETRv2Head_version2:
            convert_dict = {
        ...
        for img_id in range(batch_size):
            for cam_id in range(num_cams):
                ...
                for bbox_id in range(num_bbox):
                    min_x = np.min(imgfov_pts_2d[bbox_id, :, 0])
                    max_x = np.max(imgfov_pts_2d[bbox_id, :, 0])
                    min_y = np.min(imgfov_pts_2d[bbox_id, :, 1])
                    max_y = np.max(imgfov_pts_2d[bbox_id, :, 1])
