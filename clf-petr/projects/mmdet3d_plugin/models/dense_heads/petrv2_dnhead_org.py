...
class PETRv2DNHead(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is PETRv2DNHead):
        ...
        self.dn_for_fusion = dn_for_fusion
        self.scalar = scalar
        ...
        super(PETRv2DNHead, self).__init__(num_classes, in_channels, init_cfg = init_cfg)
    ...
    def prepare_for_dn(self, batch_size, reference_points, img_metas, objectlist_bbox=None):
        # for label denoising
        # class_names = [
                #     'car', 'truck', 'construction_vehicle', 'bus', 'trailer', 'barrier',
                #     'motorcycle', 'bicycle', 'pedestrian', 'traffic_cone'
                # ]
        # car_variants = torch.tensor([0, 1, 2, 3, 4, 5, 10]).to(reference_points.device)
        # pedestrain_variants = torch.tensor([6, 7, 8, 9, 10]).to(reference_points.device)
        # for training
        if self.training:
            ...
            # if dn used for fusion, then drop some gt boxes to simulate the object list
            # if self.dn_for_fusion:
            #     # Sample a random drop rate from (0, max_drop_rate)
            #     drop_prob = torch.rand(1).item() * self.max_drop_rate
            #     # Generate a random mask with True for elements to keep
            #     for i in range(len(targets)):
            #         keep_mask = torch.rand(targets[i].shape[0]) > drop_prob
            #         keep_mask = keep_mask.to(reference_points.device)
            #         # torch.distributed.broadcast(keep_mask, src=0)
            #         t = targets[i]
            #         l = labels[i]
            #         targets[i] = t[keep_mask]
            #         labels[i] = l[keep_mask]
            ...
            # 用在 gt labels上加上噪声
            known_bbox_center = known_bboxs[:, :3].clone() # position3d
            known_bbox_scale = known_bboxs[:, 3:6].clone() # size3d
            ...
            if self.bbox_noise_scale > 0:
                ...
                # in Nuscenes, there are 10 classes, so we add 10 to the labels, which means class label unknown
                # known_labels[mask] = self.num_classes
                # Anthor hack for nuscenes to add noise, only similar classes mixed with each other
                mask = torch.norm(rand_prob, 2, 1) > self.split
                known_labels[mask] = self.num_classes
                # car_var_mask = known_labels < 6
                # pedestrain_var_mask = known_labels >= 6
                # car_rand_values = car_variants[torch.randint(0, 7, (known_labels.shape[0],))].to(reference_points.device)
                # pedestrain_rand_values = pedestrain_variants[torch.randint(0, 5, (known_labels.shape[0],))].to(reference_points.device)
                # torch.distributed.broadcast(car_rand_values, src=0)
                # torch.distributed.broadcast(pedestrain_rand_values, src=0)
                # known_labels[car_var_mask & mask] = car_rand_values[car_var_mask & mask]
                # known_labels[pedestrain_var_mask & mask] = pedestrain_rand_values[pedestrain_var_mask & mask]
            ...
            dn_seed = np.random.rand()
            if dn_seed < self.dn_for_fusion:
                # 设置 attention mask 以避免跨组泄露真实信息
        ...
        # for testing    
            ...
            attn_mask = None
            if not self.dn_for_fusion:
                padded_reference_points = reference_points.unsqueeze(0).repeat(batch_size, 1, 1)
                mask_dict = None
            else:
                if objectlist_bbox is not None:
                    # generate only one group of queries with input object list
                    targets = [objectlist_bbox[:, :-1] for objectlist_bbox in objectlist_bbox]
                    labels = [objectlist_bbox[:, -1].view(-1).long() for objectlist_bbox in objectlist_bbox]
                    known = [(torch.ones_like(t)).cuda() for t in labels]
                    know_idx = known
                    unmask_bbox = unmask_label = torch.cat(known)
                    known_num = [t.size(0) for t in targets]
                    ...
                    known_bid = batch_idx.repeat(self.scalar, 1).view(-1)
                    known_bboxs = boxes.repeat(self.scalar, 1).to(reference_points.device)
                    # since object list is noisy, we do not add noise to it
                    # 用在 gt labels上加上噪声
                    ...
                    # generate only one group of queries with ground truth
                    targets = [torch.cat((img_meta['gt_bboxes_3d']._data.gravity_center, img_meta['gt_bboxes_3d']._data.tensor[:, 3:]),dim=1) for img_meta in img_metas ]
                    labels = [img_meta['gt_labels_3d']._data for img_meta in img_metas ]
                    # if dn used for fusion, then drop some gt boxes to simulate the object list
                    known = [(torch.ones_like(t)).cuda() for t in labels]
    ...
    def prepare_for_dn_in_processing(self, batch_size, reference_points, img_metas):
        # denoising and testing:
        if not self.dn_for_fusion and not self.training:
            padded_reference_points = reference_points.unsqueeze(0).repeat(batch_size, 1, 1)
            mask_dict = None
            attn_mask = None
            return padded_reference_points, attn_mask, mask_dict
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is PETRv2DNHead:
            convert_dict = {
    ...
    def forward(self, mlvl_feats, img_metas,objectlist_bbox=None):
        """Forward function.
        Args:
