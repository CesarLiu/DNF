...
class PETRv2Head_version4(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is PETRv2Head_version4):
        ...
        super(PETRv2Head_version4, self).__init__(num_classes, in_channels, init_cfg = init_cfg)
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is PETRv2Head_version4:
            convert_dict = {
    ...
    def forward(self, mlvl_feats, img_metas, objectlist_bbox, training_model):
        ...
        if training_model:
            ...
            #gaussian_masks = torch.zeros(batch_size, num_cams, img_pos.size(1)).to(x.device)
            for img_id in range(batch_size):
                if objectlist_bbox[img_id][0, 0, 0] == 1e-5:
                    #gaussian_masks[img_id, :, :] = 1
                    continue
                for cam_id in range(num_cams):
                    img_h, img_w, _ = img_metas[img_id]['img_shape'][cam_id]                 
                    ...
                    #gaussian_masks[img_id, cam_id, :] = gaussian_mask
                    for i in range(centers.size(0)):
                        gaussian_masks[img_id, cam_id, :]  += gaussian_mask[i]
                    gaussian_masks[img_id, cam_id, :] /= gaussian_masks[img_id, cam_id, :].max()
                    #gaussian_masks[img_id, cam_id, gaussian_masks[img_id, cam_id, :] < torch.finfo(torch.float32).eps] = 0
        ...
        #gaussian_masks = gaussian_masks.unsqueeze(1).repeat(1, query_embeds.size(0), 1)  # 扩展为 [bs, query_embeds.size(0), n*h*w]
        gaussian_masks = gaussian_masks.unsqueeze(1).expand(-1, query_embeds.size(0), -1)  # 扩展为 [bs, query_embeds.size(0), n*h*w]                  
        #gaussian_masks = gaussian_masks.squeeze(0)  # 去掉批大小维度，结果形状为 [pos_embed.size(0), n*h*w]

        gaussian_masks = (gaussian_masks+1).log()
        #########################  
        reference_points = reference_points.unsqueeze(0).repeat(batch_size, 1, 1) #.sigmoid()
        outs_dec, _ = self.transformer(x, masks, query_embeds, pos_embed, gaussian_masks, self.reg_branches)
