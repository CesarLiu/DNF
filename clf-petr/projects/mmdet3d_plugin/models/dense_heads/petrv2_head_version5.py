...
class PETRv2Head_version5(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is PETRv2Head_version5):
        ...
        super(PETRv2Head_version5, self).__init__(num_classes, in_channels, init_cfg = init_cfg)
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is PETRv2Head_version5:
            convert_dict = {
    ...
    def forward(self, mlvl_feats, img_metas, objectlist_bbox, training_model):
        ...
        if training_model:
            for img_id in range(batch_size):
                objectlist_index_img = []
                if objectlist_bbox[img_id][0, 0, 0] == 1e-5:
                    objectlist_index_img.append(0)
                    continue
                ...
                for cam_id in range(num_cams):
                    ...
                    center_ys_feat = center_ys * x.shape[3] / img_h
                    center_xs_feat = center_xs * x.shape[4] / img_w
                    for num_id1 in range(center_xs.shape[0]):
                        objectlist_index_img.append(int((cam_id+1) * center_xs_feat[num_id1] * center_ys_feat[num_id1]))

                    #corner_s = imgfov_pts_2d[on_the_image].reshape(-1,2)
                    corner_s = imgfov_pts_2d[on_the_image].reshape(-1,2)
                    on_the_image_corner = (corner_s[:, 0] > 0) * (corner_s[:, 0] < input_img_w) * (corner_s[:, 1] > 0) * (corner_s[:, 1] < input_img_h)
                    on_the_image_corner = on_the_image_corner.squeeze() 
                    corner_xys = corner_s[on_the_image_corner]   
                    corner_xys[:, 1] = corner_xys[:, 1] * x.shape[3] / img_h
                    corner_xys[:, 0] = corner_xys[:, 0] * x.shape[4] / img_w
                    for num_id2 in range(corner_xys.shape[0]):
                        objectlist_index_img.append(int((cam_id+1) * corner_xys[num_id2, 1] * corner_xys[num_id2, 0]))
                #objectlist_index_img = objectlist_index_img[:100]
                objectlist_index.append(objectlist_index_img)
            #objectlist_index = np.asarray(objectlist_index)
            objectlist_index = torch.tensor(objectlist_index, dtype=torch.int64).to(x.device)
        #objectlist_index [bs, num_obj]
           
        #########################  
        reference_points = reference_points.unsqueeze(0).repeat(batch_size, 1, 1) #.sigmoid()
        outs_dec, _ = self.transformer(x, masks, query_embeds, pos_embed, objectlist_index, training_model, self.reg_branches)
