...
class PETRDNTransformer(BaseModule):
    ...
    def forward(self, x, mask, query_embed, pos_embed, attn_masks=None, reg_branch=None):
        ...
        Args:
            ...
            pos_embed (Tensor): The positional encoding for encoder and
                decoder, with the same shape as `x`.
            attn_masks (List[Tensor] | Tensor | None): List of attention masks or single mask:
                - For list: [self_attn_mask, cross_attn_mask]
                - For single: Will be treated as self attention mask
        Returns:
            tuple[Tensor]: results of decoder containing the following tensor.
                - out_dec: Output from decoder. If return_intermediate_dec \
                      is True output has shape [num_dec_layers, bs,
        ...
        mask = mask.view(bs, -1)  # [bs, n, h, w] -> [bs, n*h*w]
        target = torch.zeros_like(query_embed)
        decoder_attn_masks = [None, None]
        if attn_masks is not None:
            if not isinstance(attn_masks, list):
                # Single mask -> treat as self attention mask in DN
                decoder_attn_masks[0] = attn_masks
            else:
