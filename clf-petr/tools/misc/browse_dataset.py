...
import sys
sys.path.insert(0, '/home/jguo/PETR/PETR')
import mmcv
import numpy as np
...
def show_proj_bbox_img(input, out_dir, show=False, is_nus_mono=False):
    ...
    img_metas = input['img_metas']._data
    img_f = input['img']._data.numpy()
    # imgs = input['img']._data
    # mean = torch.from_numpy(img_metas['img_norm_cfg']['mean']).view(1, -1, 1, 1).to(imgs.device)
    # std = torch.from_numpy(img_metas['img_norm_cfg']['std']).view(1, -1, 1, 1).to(imgs.device)

    # imgs = imgs.clone()*std + mean
    # img_f = imgs.numpy()
    objectlist=input['objectlist_bbox']._data
    ...
    for i in range(img_f.shape[0]):
        ...
        if gt_bboxes.tensor.shape[0] == 0:
            gt_bboxes = None
        filename = Path(img_metas['filename'][i]).name
    ...
    if hasattr(cfg, 'plugin'):
        if cfg.plugin:
            ...
            if hasattr(cfg, 'plugin_dir'):
                ...
                _module_dir = osp.dirname(args.config)
                _module_dir = _module_dir.split('/')
                _module_path = _module_dir[0]
                for m in _module_dir[1:]:
                    _module_path = _module_path + '.' + m
    ...
    for input in dataset:
        ...
        if vis_task in ['multi_modality-det', 'mono-det']:
            ...
            show_proj_bbox_img(
                ...
                args.output_dir,
                show=args.online,
                is_nus_mono=(dataset_type == 'CustomNuScenesDataset'))
