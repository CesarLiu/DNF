...
def plot_rect3d_on_img(img,
    ...
    for i in range(num_rects):
        ...
        for start, end in line_indices:
            pt1 = (np.clip(corners[start, 0], 0, img.shape[1]-1), np.clip(corners[start, 1], 0, img.shape[0]-1))
            pt2 = (np.clip(corners[end, 0], 0, img.shape[1]-1), np.clip(corners[end, 1], 0, img.shape[0]-1))
            cv2.line(img, pt1,
                     pt2, color, thickness,
                     cv2.LINE_AA)
...
def draw_lidartensor_on_img(bboxes3d,
                             raw_img,
                             lidar2img_rt,
                             img_metas,
                             color=(0, 255, 0),
