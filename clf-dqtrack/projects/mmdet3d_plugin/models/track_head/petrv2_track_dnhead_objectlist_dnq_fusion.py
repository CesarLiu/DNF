...
# Modified from PETR (https://github.com/megvii-research/PETR/blob/main/projects/mmdet3d_plugin/models/dense_heads/petrv2_head.py)
# ------------------------------------------------------------------------
...
class PETRv2TrackDNHead_objectlist_dnq_fusion(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is PETRv2TrackDNHead_objectlist_dnq_fusion):
        ...
        super(PETRv2TrackDNHead_objectlist_dnq_fusion, self).__init__(num_classes, in_channels, init_cfg = init_cfg)
    ...
    def prepare_for_dn(self, batch_size, reference_points, img_metas=None, objectlist_bbox=None):
        # objectlist_bbox = objectlist_bbox[0]
        ...
        if self.training and img_metas is not None:
            gt_targets = [torch.cat((img_meta['gt_bboxes_3d'].gravity_center, img_meta['gt_bboxes_3d'].tensor[:, 3:]),dim=1) for img_meta in img_metas ]
            gt_labels = [img_meta['gt_labels_3d'] for img_meta in img_metas ]
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is PETRv2TrackDNHead_objectlist_dnq_fusion:
            convert_dict = {
