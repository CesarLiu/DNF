...
class PETRTrackerDQ_objectlist_dnq_static(MVXTwoStageDetector):
    """PETRTrackerDQ."""
    def __init__(self,
                 use_grid_mask=False,
                 ...
                 load_img=None,
                 load_pts=None):
        super(PETRTrackerDQ_objectlist_dnq_static,
        ...
        self.prop_dnq = tracker_cfg.get("prop_dnq", False)
    ...
    def forward_detector(self, img, img_metas, detector, objectlist_bbox, objectlist_bbox_corner):
        img_feats = self.extract_feat(img=img, img_metas=img_metas)
        pred_dict = self.pts_bbox_head(img_feats, img_metas,objectlist_bbox, objectlist_bbox_corner)

        return pred_dict, detector

    def forward_single(self, img, detector, img_metas, objectlist_bbox=None, objectlist_bbox_corner=None, depth_map=None, frame_idx=0):
        pred_dict, detector = self.forward_detector(img, img_metas, detector,objectlist_bbox, objectlist_bbox_corner)
        ...
        # extract denosie query feature and outputs
        if self.prop_dnq and pred_dict['dn_mask_dict'] is not None:
            dn_mask_dict = pred_dict['dn_mask_dict']
            dn_query_feat = dn_mask_dict['output_known_query_feat']
            dn_output_classes, dn_output_coords = dn_mask_dict['output_known_lbs_bboxes']
            dn_query_feat = dn_query_feat[0].float()
            dn_output_classes = dn_output_classes[:,0].float()
            dn_output_coords = dn_output_coords[:,0].float()
            dn_scores = dn_output_classes[-1].sigmoid().max(dim=-1).values
            # regroup dn and det outputs according to scalar
            group_scalar = self.tracker_cfg.get('scalar', 10)
            obj_list_num = dn_scores.shape[0] // group_scalar
            # reshape dn outputs
            dn_output_classes = dn_output_classes.reshape(dn_output_classes.shape[0], group_scalar,obj_list_num, dn_output_classes.shape[-1])
            dn_output_coords = dn_output_coords.reshape(dn_output_coords.shape[0], group_scalar, obj_list_num, dn_output_coords.shape[-1])
            dn_query_feat = dn_query_feat.reshape(group_scalar, obj_list_num, dn_query_feat.shape[-1])
            dn_scores = dn_scores.reshape(group_scalar, obj_list_num)
            # calculate mean along the dimension of scalar
            dn_output_classes = dn_output_classes.mean(dim=1)
            dn_output_coords = dn_output_coords.mean(dim=1)
            dn_query_feat = dn_query_feat.mean(dim=0)
            dn_scores = dn_scores.mean(dim=0)
            # Identify the indices of the obj_list_num minimal values in det_scores
            _, min_indices = torch.topk(det_scores, obj_list_num, largest=False)
            # replace the obj_list_num minimal values in output with the corresponding values in dn outputs
            output_classes[:, min_indices, :] = dn_output_classes
            output_coords[:, min_indices, :] = dn_output_coords
            last_query_feat[min_indices, :] = dn_query_feat
            det_scores[min_indices] = dn_scores
    ...
    def forward_train(self,
                      ...
                      objectlist_bbox=None, 
                      objectlist_bbox_corner=None):
        ...
        for batch_idx in range(batch_num):
            ...
            for frame_idx in range(num_frame):
                ...
                objectlist_bbox_single = [deepcopy(objectlist_bbox[batch_idx])]
                objectlist_bbox_single[0] = objectlist_bbox[0][frame_idx]
                objectlist_bbox_corner_single = [deepcopy(objectlist_bbox_corner[batch_idx])]
                objectlist_bbox_corner_single[0] = objectlist_bbox_corner[0][frame_idx] 
                ...
                if self.tracker_cfg.get('train_track_only', False):
                    with torch.no_grad():
                        ...
                        detector, loss_single, info_single = self.forward_single(
                                                                    ...
                                                                    detector, 
                                                                    img_metas_single,
                                                                    objectlist_bbox_single, 
                                                                    objectlist_bbox_corner_single,
                                                                    depth_map,
                                                                    frame_idx)
                else:
                    # calculate detection loss and matched pairs for each frame
    ...
    def forward_test(self, 
        ...
        for frame_idx in range(num_frame):
            ...
            objectlist_bbox_single = objectlist_bbox[0]
            objectlist_bbox_corner_single = objectlist_bbox_corner[0]

            detector = self.forward_single(img_single, detector, img_metas_single, objectlist_bbox_single, objectlist_bbox_corner_single, frame_idx=frame_idx)
