...
class NuScenesDatasetPETRTrack_withpred(Custom3DDataset):
    r"""NuScenes Dataset.
    ...
    def get_data_info(self, index):
        ...
        l2g_r_mat = e2g_r_mat @ l2e_r_mat  # [3, 3]
        l2g_t = l2e_t @ e2g_r_mat.T + e2g_t  # [1, 3]

        # TODO: Check the trans mat
        # lidar to ego system
        lidar2ego_rt = np.eye(4)
        lidar2ego_rt[:3, :3] = l2e_r_mat
        lidar2ego_rt[:3, 3] = np.array(l2e_t)
        # lidar to global system
        lidar2global_rt = np.eye(4)
        lidar2global_rt[:3, :3] = l2g_r_mat
        lidar2global_rt[:3, 3] = np.array(l2g_t)
        ...
        input_dict.update(
            dict(
                ...
                lidar2ego=lidar2ego_rt.astype(np.float32),
                lidar2global=lidar2global_rt.astype(np.float32))
        ...
        if  self.test_mode:
            annos = self.get_ann_info_test(index)
            input_dict['ann_info_test'] = annos
    ...
    def get_ann_info_test(self, index):
        """Get annotation info according to the given index.
    ...
    def prepare_train_data(self, index):
        ...
        if self.force_continuous and 'scene_token' in self.data_infos[start]:
            reassign = False
            try:
                if end > len(self.data_infos):
                    reassign = True
                elif self.data_infos[start]['scene_token'] != \
                self.data_infos[end]['scene_token']:
                    reassign = True
            except:
                print("END:{}, Total NUM:{}".format(end, len(self.data_infos)))
    ...
    def _format_bbox(self, results, jsonfile_prefix=None):
        ...
        """
        nusc_annos = {}
        if  results[0].get('eval_det', False):
            ...
            for i, box in enumerate(boxes):
                ...
                if np.sqrt(box.velocity[0]**2 + box.velocity[1]**2) > 0.2:
                    if name in [
                        ...
                        attr = NuScenesDatasetPETRTrack_withpred.DefaultAttribute[name]
                else:
                ...
                # _center = box.center.tolist()
                # change from ground height to center height
                # _center[2] = _center[2] + (box.wlh.tolist()[2] / 2.0)
                nusc_anno = dict(
